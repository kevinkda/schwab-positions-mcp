...
from schwab_positions_mcp import cache_backend as cb
from schwab_positions_mcp.cache_backend import (
    ENV_CACHE_BACKEND,
    ENV_CLICKHOUSE_URL,
    ClickHouseBackend,
...
class TestClickHouseBackendConnect:
    ...
    def test_import_failure_raises_friendly(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with pytest.raises(ClickHouseNotInstalledError) as exc:
            ClickHouseBackend()
        assert "pip install schwab-positions-mcp[clickhouse]" in exc.value.hint


class TestClickHouseNotInstalledError:
    def test_hint_message(self) -> None:
        err = ClickHouseNotInstalledError()
        assert "pip install schwab-positions-mcp[clickhouse]" in err.hint
...
def test_backends_satisfy_protocol() -> None:
    from schwab_positions_mcp.cache_backend import CacheBackend
