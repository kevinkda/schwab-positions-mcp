...
* Layer 1 — :data:`schwab_positions_mcp.client._READ_ONLY_METHODS` allow list
...
added a mutation method name to the allow list (Layer 1 regression) or routed
...
# ---------------------------------------------------------------------------
# Allow-listed read-only methods proxy through correctly
# ---------------------------------------------------------------------------


def test_allow_list_methods_proxy_to_underlying(
    readonly_client: ReadOnlySchwabClient,
    mock_schwab_client: MagicMock,
) -> None:
    """Every method in the allow list must forward verbatim to the wrapped client."""
...
def test_allow_list_contains_expected_read_methods() -> None:
    """Sanity: the allow list matches what docs/SECURITY.md advertises."""
    ...
    assert "get_order" in _READ_ONLY_METHODS
    assert "get_transaction" in _READ_ONLY_METHODS
...
def test_arbitrary_mutation_method_raises_not_implemented(
    ...
    """Any non-allow-listed attribute access MUST raise ``NotImplementedError``."""
    with pytest.raises(NotImplementedError):
