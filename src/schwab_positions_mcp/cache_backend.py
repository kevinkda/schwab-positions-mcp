"""Pluggable cache backends for schwab-positions-mcp (v0.7 T0 template).
...
* :class:`ClickHouseBackend` — the **opt-in** backend.  Enabled only when
  ``pip install schwab-positions-mcp[clickhouse]`` is present *and*
  ``SCHWAB_POSITIONS_CLICKHOUSE_URL`` is configured.  ``clickhouse_connect`` is
...
* :func:`get_cache_backend` — the factory.  Selects the backend from
  ``SCHWAB_POSITIONS_CACHE_BACKEND`` (``memory`` | ``clickhouse``, default
  ``memory``).
...
ENV_CACHE_BACKEND: Final[str] = "SCHWAB_POSITIONS_CACHE_BACKEND"
ENV_CLICKHOUSE_URL: Final[str] = "SCHWAB_POSITIONS_CLICKHOUSE_URL"
...
REQUIRES_CLICKHOUSE_HINT: Final[str] = (
    ...
    "Install it with `pip install schwab-positions-mcp[clickhouse]` and set "
    "SCHWAB_POSITIONS_CLICKHOUSE_URL plus SCHWAB_POSITIONS_CACHE_BACKEND=clickhouse. "
...
class ClickHouseNotInstalledError(RuntimeError):
    ...
    def __init__(self, original: Exception | None = None) -> None:
        self.hint = (
            ...
            "`pip install schwab-positions-mcp[clickhouse]`."
        )
...
class ClickHouseBackend:
    ...
    ``SCHWAB_POSITIONS_CLICKHOUSE_URL`` is configured.  Connection / query
    ...
    _RESPONSE_TABLE: Final[str] = "schwab_positions_response_cache"
    _TIMESERIES_TABLE: Final[str] = "schwab_positions_timeseries"
...
def get_cache_backend() -> CacheBackend:
    ...
    Honors ``SCHWAB_POSITIONS_CACHE_BACKEND`` (``memory`` | ``clickhouse``).
